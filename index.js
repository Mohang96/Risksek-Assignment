...
class Library {
    ...
    addBook(book){
        ...
        if (fileFormat === undefined) {
            this.books = [...this.books, {"title": book.title, "author": book.author, "ISBN": book.ISBN}];  //updating books array by keeping the state consistent
        } else {
            this.books = [...this.books, {"title": book.title, "author": book.author, "ISBN": book.ISBN, "fileFormat": book.fileFormat}];  //updating books array by keeping the state consistent
        }
    }
    ...
    searchByTitle(title){
        try {
            let searchedBooks = this.books.filter(eachBook => eachBook.title.toLowerCase().includes(title.toLowerCase()));
...
Library.prototype.deleteBook = function(ISBN) {   //adding delete method using prototype object
...
let eBook1 = new EBook("The Catcher in the Rye", "J.D.Salinger", 9780316769480, "PDF");
...
app.delete("/deleteBook", (request, response) => {
    try {
        const booksArray = library1.books;
        if (booksArray.length === 0) {
            throw new Error("The Library is Empty and there are No Books for deletion");  //throws an error when the library is empty
        }
